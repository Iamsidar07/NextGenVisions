...
const Modal = ({ post, setOpenImageId }: ModalProps) => {
  ...
  return (
    <div className=" overflow-hidden fixed flex  items-end md:items-center justify-center top-0 bottom-0 left-0 right-0 inset-0 bg-black/80 z-20 p-2">
      <div
        className="w-5 h-5 rounded gradientbg1 absolute top-1 left-1 m-2  items-center justify-center cursor-pointer z-50 flex"
        ...
        <div className="w-full py-5 sm:py-0 md:max-w-sm mb-[100%] md:mb-0 ">
          <div className="px-2">
            <div className="gradientbg1 rounded sm:rounded p-2 md:p-4 shadow-sm border   ">
              <p className="prompt mb-3 ">{prompt}</p>
              <div className="flex items-center gap-1">
                <div
                  className="rounded px-3 py-1.5   border text-black w-fit cursor-pointer hover:text-white hover:bg-black transition-all duration-150 ease-linear flex items-center"
                  onClick={() => copyToClipboard(prompt)}
                >
                  <FiCopy size={16} className="mr-1" />
                  <p className="hidden md:inline-block"> Copy prompt</p>
                </div>
                <div
                  className="rounded px-3 py-1.5   border text-black w-fit cursor-pointer hover:text-white hover:bg-black transition-all duration-150 ease-linear flex items-center ml-1"
                  onClick={() => copyToClipboard(imageSrc)}
                >
                  <FiLink size={16} className="mr-1" />
                  <p className="hidden md:inline-block"> Copy Link</p>
                </div>
                <div
                  className="rounded px-4 py-1.5 space-x-2  bg-black text-white hover:bg-white hover:text-black transition-all duration-150 ease-linear  w-fit cursor-pointer flex items-center border"
                  onClick={() => downloadImage(_id, photo)}
                >
                  <BsDownload className="h-6" />
                </div>
              </div>
            </div>
